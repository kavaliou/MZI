...
class AES:
    def __init__(self, z):
        ...
        self.R_CON = [0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
        ...
        self.expandedKey = self._expandKey(z, n_k)
    ...
    def encrypt(self, x):  # returns array of bytes
        ...
        state = [[0 for _ in range(self.Nb)] for _ in range(self.Nb)]

        byte_index = 0
        for i, row in enumerate(state):
            for j, val in enumerate(row):
                state[j][i] = x[byte_index]
                byte_index += 1

        state = self._addRoundKey(state, self.expandedKey[0:self.Nb])

        for round_number in range(1, self.Nr):
            state = self._subBytes(state)
            state = self._shiftRows(state)
            state = self._mixColumns(state)
            state = self._addRoundKey(
                state,
                self.expandedKey[round_number * self.Nb:((round_number + 1) * self.Nb - 1) + 1]
            )
    ...
    def decrypt(self, y):  # returns array of bytes
       ...
       Decrypts a 128-bit (16-byte) ciphertext block using this AES cipher.
        ...
        for i in range(len(state)):
            for j in range(len(state[0])):
                cipher_text.append(int(state[j][i]))
    ...
    def _expandKey(self, key, Nk):
        ...
        while i < self.Nb * (self.Nr + 1):
            ...
            if i % Nk == 0:
                ...
                x = temp[:]
                x[0] = x[0] ^ self.R_CON[i / Nk]
                temp[0] = temp[0] ^ self.R_CON[i / Nk]
